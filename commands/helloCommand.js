...
const helloCommand = (message, user) => {
  ...
  if (!user['badges-raw']) {
    return `Hey, ${user[`display-name`]} episteWave !`;
  }

  // If the user is a mod
  if (user['mod'] === true) {
    return `Hey, ${user[`display-name`]} episteWave ! Thank you for being an awesome mod :D`;
  }
  ...
  if (user['badges-raw']) {
      ...
      if (!user['badges-raw'].includes("broadcaster") ||
          ...
          return `Hey ${user[`display-name`]} episteWave ! Nice badge(s)!`;
      }

      // If the user is the broadcaster
      if (user['badges-raw'].includes("broadcaster")) {
          return `Hey ${user[`display-name`]} episteWave ! Thank you for being an awesome broadcaster!`;
        // If the user is a subscriber
      } else if (user['badges-raw'].includes("subscriber")) {
          return `Hey ${user[`display-name`]} episteWave ! Thank you for being a sub!`;
      }
    }
}
